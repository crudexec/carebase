...
const fetchUserForms = async (req, res, next) => {
    ...
    const offerLetterRepository = (0, typeorm_1.getRepository)(letter_1.OfferLetter);
    try {
        ...
        const personalInformationHepatitisBForm = (await personalInformationHepatitisBFormRepository.findOne({ where: { user_id } })) || {};
        ...
        const pneumococcalSignatureForm = (await pneumococcalSignatureFormRepository.findOne({ where: { signed_by: user_id } })) || {};
        const pneumococcalVaccinationFullForm = (await pneumococcalVaccinationFullFormRepository.findOne({ where: { user_id } })) || {};
        ...
        const cjisEmployeeInformation = (await (0, typeorm_1.getRepository)(cjisEmployeeInformation_1.CJISEmployeeInformation).findOne({ where: { user_id } })) || {};
        const cjisSignatureForm = (await (0, typeorm_1.getRepository)(cjisSignature_1.CJISSignatureForm).findOne({ where: { signed_by: user_id } })) || {};
        const cjisPreRegistrationForm = (await cjisPreRegistrationFormRepository.findOne({ where: { user_id } })) || {};
        const offerLetter = (await offerLetterRepository.findOne({
            where: { user_id },
            order: {
                created_at: 'DESC',
            },
...
exports.fetchUserForms = fetchUserForms;
//# sourceMappingURL=fetchUserForms.js.map