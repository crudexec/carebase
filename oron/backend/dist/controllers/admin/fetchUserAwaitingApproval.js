...
exports.fetchUserAwaitingForms = void 0;
...
const fetchUserAwaitingForms = async (req, res, next) => {
    ...
    try {
        ...
        const userBioData = await userBioDataRepository.findOne({
            where: { user_id: user_id, status: genericEnums_1.Status.AWAITING_APPROVAL },
        });
        ...
        const employeeDemographicInformation = await employeeDemographicRepository.findOne({
            where: { user_id, status: genericEnums_1.Status.AWAITING_APPROVAL },
        });
        const fluFullForm = await fluFullFormRepository.findOne({ where: { user_id, status: genericEnums_1.Status.AWAITING_APPROVAL } });
        ...
        const i9Form = await i9FormRepository.findOne({ where: { owner: user_id, status: genericEnums_1.Status.AWAITING_APPROVAL } });
        ...
        const mmrFullForm = await mmrFullFormRepository.findOne({ where: { user_id, status: genericEnums_1.Status.AWAITING_APPROVAL } });
        ...
        const tuberculosisFullForm = (await tuberculosisFullFormRepository.findOne({ where: { owner: user_id, status: genericEnums_1.Status.AWAITING_APPROVAL } })) ||
            {};
        ...
        const referenceForm = await referenceFormRepository.findOne({
            where: { user_id, status: genericEnums_1.Status.AWAITING_APPROVAL },
        });
        const cjisFullForm = await cjisFullFormRepository.findOne({ where: { user_id, status: genericEnums_1.Status.AWAITING_APPROVAL } });
        ...
        if (!(0, lodash_1.isEmpty)(emergencyContactInformation)) {
            formsData['employeeDemographicForm'] = {
                emergencyContactInformation,
                employeeDemographicInformation,
            };
        }
        if (!(0, lodash_1.isEmpty)(fluFullForm)) {
            formsData['fluForm'] = {
                fluFullForm,
        ...
        if (!(0, lodash_1.isEmpty)(hepatitisBFullForm)) {
        ...
        if (!(0, lodash_1.isEmpty)(mmrFullForm)) {
            formsData['mmrForm'] = {
                mmrFullForm,
                mmrSignatureForm,
                mmrAttestationForm,
        ...
        if (!(0, lodash_1.isEmpty)(pneumococcalVaccinationFullForm)) {
        ...
        if (!(0, lodash_1.isEmpty)(tuberculosisFullForm)) {
        ...
        if (!(0, lodash_1.isEmpty)(varicellaFullForm)) {
        ...
        if (!(0, lodash_1.isEmpty)(cjisFullForm)) {
            formsData['cjisForm'] = {
                cjisFullForm,
...
exports.fetchUserAwaitingForms = fetchUserAwaitingForms;
//# sourceMappingURL=fetchUserAwaitingApproval.js.map