"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReviewPneumococcalForm = void 0;
const typeorm_1 = require("typeorm");
const pneumococcalFullForm_1 = require("orm/entities/PneumoccalVaccinationForm/pneumococcalFullForm");
const User_1 = require("orm/entities/User");
const genericEnums_1 = require("types/genericEnums");
const emailService_1 = require("utils/emailService");
const CustomError_1 = require("utils/response/custom-error/CustomError");
const ReviewPneumococcalForm = async (req, res, next) => {
    const pneumococcalFormRepository = (0, typeorm_1.getRepository)(pneumococcalFullForm_1.PneumococcalVaccinationFullForm);
    const userRepository = (0, typeorm_1.getRepository)(User_1.User);
    const form_id = req.params.id;
    const { review_notes } = req.body;
    try {
        const pneumococcalForm = await pneumococcalFormRepository.findOne({ where: { id: form_id } });
        if (!pneumococcalForm) {
            const customError = new CustomError_1.CustomError(404, 'General', `Pneumococcal Form not found.`, [
                'Pneumococcal Form not found.',
            ]);
            return next(customError);
        }
        const user = await userRepository.findOne({ where: { id: pneumococcalForm.user_id } });
        await pneumococcalFormRepository.update({ id: form_id }, { status: genericEnums_1.Status.REVIEWED, review_notes });
        await (0, emailService_1.SendReviewEmail)(user.first_name, `Pneumococcal Form`, review_notes, String(user.email));
        return res.customSuccess(200, 'Pneumococcal Form successfully reviewed.', pneumococcalForm);
    }
    catch (err) {
        const customError = new CustomError_1.CustomError(400, 'Raw', 'Error', null, err);
        return next(customError);
    }
};
exports.ReviewPneumococcalForm = ReviewPneumococcalForm;
//# sourceMappingURL=reviewPneumococcal.js.map