...
export const fetchUserForms = async (req: RequestWithJwtPayload, res: Response, next: NextFunction) => {
  ...
  const offerLetterRepository = getRepository(OfferLetter);

  try {
    const user = await userRepository.findOne({ where: { id: user_id, role: Role.STANDARD } });

    const userBioData = (await userBioDataRepository.findOne({ where: { user_id: user_id } })) || {};

    const emergencyContactInformation =
      (await emergencyContactInformationRepository.findOne({ where: { user_id } })) || {};
    const employeeDemographicInformation = (await employeeDemographicRepository.findOne({ where: { user_id } })) || {};
    const fluFullForm = (await fluFullFormRepository.findOne({ user_id })) || {};
    ...
    const personalInformationHepatitisBForm =
      (await personalInformationHepatitisBFormRepository.findOne({ where: { user_id } })) || {};
    const hepatitisBSignatureForm =
    ...
    // const influenzaVaccinationDeclinationFullForm =
    //   (await influenzaVaccinationDeclinationFullFormRepository.findOne({ where: { user_id } })) || {};
    ...
    const n95FitFullForm = (await n95FitFullFormRepository.findOne({ where: { user_id } })) || {};
    ...
    const pneumococcalSignatureForm =
      (await pneumococcalSignatureFormRepository.findOne({ where: { signed_by: user_id } })) || {};
    const pneumococcalVaccinationFullForm =
      (await pneumococcalVaccinationFullFormRepository.findOne({ where: { user_id } })) || {};
    const pneumococcalVaccinationForm =
      (await pneumococcalVaccinationFormRepository.findOne({ where: { user_id } })) || {};
    ...
    const cjisEmployeeInformation =
      (await getRepository(CJISEmployeeInformation).findOne({ where: { user_id } })) || {};
    const cjisSignatureForm = (await getRepository(CJISSignatureForm).findOne({ where: { signed_by: user_id } })) || {};
    const cjisPreRegistrationForm = (await cjisPreRegistrationFormRepository.findOne({ where: { user_id } })) || {};

    const offerLetter =
      (await offerLetterRepository.findOne({
        where: { user_id },
        order: {
          created_at: 'DESC',
        },
