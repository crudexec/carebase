...
export const submitReferenceForm = async (req: RequestWithJwtPayload, res: Response, next: NextFunction) => {
  try {
    ...
    await referenceFormRepository.update(referenceForm.id, { status: Status.AWAITING_APPROVAL });

    return res.customSuccess(200, 'Reference form successfully submitted.', referenceForm);
  } catch (err) {
    const customError = new CustomError(400, 'Raw', 'Error while submitting reference form', null, err);
    return next(customError);
  }
};
