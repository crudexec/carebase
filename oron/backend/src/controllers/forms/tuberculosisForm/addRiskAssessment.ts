...
export const fillTbRiskAssessmentForm = async (req: RequestWithJwtPayload, res: Response, next: NextFunction) => {
  const {
    had_tb_infection,
  ...
  try {
    ...
    if (tuberculosisMantouxForm) {
      const customError = new CustomError(400, 'General', 'Tuberculosis Mantoux form already exists', [
