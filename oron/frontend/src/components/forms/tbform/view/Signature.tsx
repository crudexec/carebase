...
import { handleTBFormSignatureSubmission } from "@/actions/forms";
...
const Signature = ({
  ...
  const { mutate, isPending } = useCustomMutation(
    async (imageURL: string) => {
      ...
      const response = await handleTBFormSignatureSubmission(
        form,
        token,
        method
      );
  ...
  useEffect(() => {
    if (
      typeof data !== "boolean" &&
      data?.data.tuberculosisSignatureForm?.signature_data
    ) {
      setImageURL(data?.data.tuberculosisSignatureForm.signature_data ?? "");
      setSigned(true);
    }
  }, [data, signMonitor]);
  ...
  return (
    <section className="flex-1 h-fit flex flex-col gap-10 mt-5">
      <div className="flex flex-col gap-2">
        <h2 className="text-[28px] font-[600] text-[#0F172A]">Signature</h2>
        <p className="text-[16px] font-[400] text-[#334155] pb-10">
          I am aware that the Federal law provides for imprisonment and/or fines
          for false statements, or the use of false documents, in connection
          with the completion of this form. I attest, under penalty of pejury,
          that this information, including my selection of the box attesting to
          my citizenship or immigration status, is true and correct.
        </p>
