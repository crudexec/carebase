...
import { retrieveMMRVaccineAttestation } from "@/use-cases/forms";
...
const MmrVaccineAttestationFormPageWrapper = () => {
  ...
  const {
    ...
    refetch,
  } = useCustomQuery<MMRFormResponse | undefined>(
    "MMRForm",
  ...
  useEffect(() => {
    if (formStatusData && typeof formStatusData !== "boolean") {
      setStatus(formStatusData?.mmrVaccine);
    }
  }, [formStatusData]);
  ...
  useEffect(() => {
    if (!formInfoLoading && formInfo && typeof formInfo !== "boolean") {
      const attestation = formInfo.data.mmrAttestationForm;
      const information = formInfo.data.mmrEmployeeInformation;
      const signature = formInfo.data.mmrSignatureForm;
  ...
  if (typeof formInfo === "boolean") {
    ...
    reviewNote = formInfo?.data.mmrFullForm.review_notes ?? "";
  }
  ...
  return (
    <div className="w-full flex flex-col gap-5 relative">
      ...
      {status === "Approved" && (
        <FormApprovedModal formName="MMR Vaccine Attestation Form" />
      )}
        ...
        <div className="w-full flex flex-col">
          <div className="lg:fixed lg:mt-5 text-[30px] lg:z-[500] w-full xl:w-[80%] pr-10 bg-white font-[600] text-[#101828] xl:py-5 flex flex-col lg:flex-row lg:justify-between flex-wrap lg:items-center gap-5">
            <div className="flex flex-col lg:flex-row lg:items-center gap-5">
              <span>MMR Vaccine Attestation</span>
...
export default MmrVaccineAttestationFormPageWrapper;
