...
import { handleVaricellaSignatureSubmission } from "@/actions/forms";
...
const Signature = ({
  ...
  const { mutate, isPending } = useCustomMutation(
    async (imageURL: string) => {
      ...
      const response = await handleVaricellaSignatureSubmission(
        form,
        token,
        method
      );
    ...
    ["varicellaForm", "formData", "offerLetter"]
  );
  ...
  useEffect(() => {
    if (
      typeof data !== "boolean" &&
      data?.data.varicellaSignatureForm?.signature_data
    ) {
      setImageURL(data?.data.varicellaSignatureForm.signature_data ?? "");
      setSigned(true);
    }
  }, [data]);

  return (
    <section className="flex flex-col gap-10 mt-[5vh]">
      ...
      {imageURL && (
        <div className="flex flex-col gap-5 w-[250px] h-[200px] mt-5">
          <button
            ...
            onClick={() => setIsOpen(true)}
            className="flex disabled:cursor-not-allowed w-fit h-fit text-white ml-auto justify-end rounded-full bg-[#18181B] p-2"
          >
