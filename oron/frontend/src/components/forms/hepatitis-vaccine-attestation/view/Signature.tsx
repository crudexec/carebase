...
import { handleHepatitisSignatureSubmission } from "@/actions/forms";
...
const Signature = ({
  ...
  const { mutate, isPending } = useCustomMutation(
    async (imageURL: string) => {
      ...
      const response = await handleHepatitisSignatureSubmission(
        form,
        token,
        method
      );
  ...
  useEffect(() => {
    if (
      typeof data !== "boolean" &&
      data?.data.signatureInformation?.signature_data
    ) {
      setSigned(true);
      setImageURL(data?.data.signatureInformation.signature_data ?? "");
    }
  }, [data]);
