...
import { handleI9FormSignatureSubmission } from "@/actions/forms";
import {
  FormattedFormStatus,
  INineFormResponse,
...
const Signature = ({
  ...
  const { mutate, isPending } = useCustomMutation(
    async (imageURL: string) => {
      ...
      const response = await handleI9FormSignatureSubmission(
        form,
        token,
        method
      );
  ...
  useEffect(() => {
    if (typeof data !== "boolean" && data?.data.signature?.signature_data) {
      setImageURL(data?.data.signature?.signature_data ?? "");
      setSigned(true);
    }
  }, [data, signMonitor]);
