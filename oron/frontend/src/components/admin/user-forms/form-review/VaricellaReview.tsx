...
const VaricellaReview = ({
  ...
  const {
    ...
    reviewNote,
    handleChange,
  } = useSendReview("varicella", name, formID!);
    ...
    <section className="flex flex-col gap-10 py-10">
      <BreadCrumb
        links={[
          ...
          },
          {
            ...
            route: `/admin?name=${name}&id=${id}&form=varicellaVaccine&formId=${formID}`,
          },
        ]}
      />
          ...
          <FormBadge
            status={
              reviewPending
                ...
                : isPending
                ? "Approved"
                : formInfo?.status.varicellaVaccine ?? "Not Filled"
            }
          >
            {reviewPending
              ? "Correction Required"
              : isPending
              ? "Approved"
              : formInfo?.status.varicellaVaccine}
          </FormBadge>
        </div>
        <div className="flex gap-5 items-center">
          <Dialog>
            <DialogTrigger
              disabled={formInfo?.status.varicellaVaccine === "Not Filled"}
            >
              {reviewPending && (
                <button
                  disabled
              ...
              {!reviewPending && (
                ...
                >
                  <Image
                    ...
                    alt="info icon"
                  />
                  {formInfo?.status?.varicellaVaccine === "Correction Required"
                    ? "Send Another Review"
                    : "Correction Required"}
                </button>
              )}
          ...
          {!isPending && (
            <button
              disabled={
                formInfo?.status.varicellaVaccine === "Not Filled" ||
                formInfo?.status.varicellaVaccine === "Approved" ||
                hasApproved
              }
              onClick={() => mutate(name)}
            ...
            >
              <Image
                ...
                alt="approve icon"
              />
              {formInfo?.status.varicellaVaccine === "Approved" || hasApproved
                ? "Approved"
                : "Approve"}
            </button>
          )}
      ...
      {formInfo?.status?.varicellaVaccine === "Correction Required" && (
        <FormBanner
          variant="warning"
          text={
            formInfo?.forms?.varicellaVaccine?.data?.varicellaFullForm
              ?.review_notes ?? ""
          }
        />
      )}
          ...
          <div className="w-full flex h-fit flex-col gap-7">
            <div className="flex flex-col gap-3">
              ...
              <ul className="text-[14px] font-[400] text-[#0F172A] list-disc ml-5">
                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  .had_chicken_pox && <li>I already have had chickenpox</li>}

                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  .will_not_contract_chicken_pox && (
                  <li>I do not think I will contract chickenpox</li>
                )}

                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  .chicken_pox_not_serious_disease && (
                  <li>I do not think chickenpox is a serious disease</li>
                )}

                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  .side_effects_from_chicken_pox_vaccine && (
                  <li>
                    I had side effects when I was vaccinated against chickenpox
                    in the past
                  </li>
                )}
                ...
                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  ...
                  <li>
                    I will stay home if I get chickenpox so I will not spread it
                ...
                {formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                  ...
                  formInfo?.forms.varicellaVaccine.data.varicellaAttestationForm
                    ...
                    <li>
                      {
                        formInfo?.forms.varicellaVaccine.data
                          .varicellaAttestationForm.other
                      }
                    </li>
                  )}
              </ul>
          ...
          <div className="w-full flex h-fit flex-col gap-7">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-[14px] font-[400] text-[#0F172A]">
                {formInfo?.forms.varicellaVaccine.data
                  .varicellaEmployeeInformation.last_name ?? "-"}
              </p>
            </div>
            <div className="w-full flex flex-col gap-1">
              ...
              <p className="text-[14px] font-[400] text-[#0F172A]">
                {formInfo?.forms.varicellaVaccine.data
                  .varicellaEmployeeInformation.first_name ?? "-"}
              </p>
            </div>
          </div>
        </div>
...
export default VaricellaReview;
