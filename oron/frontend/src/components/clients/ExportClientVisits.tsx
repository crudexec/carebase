...
import { retrieveClientVisitExport } from "@/use-cases/clients";
...
const convertToCSV = (data: VisitLog[]): string => {
  ...
  data.forEach((visit) => {
    ...
    const staffName = `${visit.staff?.first_name ?? "-"} ${
      visit.staff?.last_name ?? "-"
    }`;
  ...
  Object.keys(groupedByDate).forEach((date) => {
    ...
    visits.forEach((visit, index) => {
      ...
      rows.push(
        `${index === 0 ? date : ""},${visit.staff},${visit.start_time},${
          visit.end_time
...
const ExportClientVisits = ({
  ...
  const handlePreview = async () => {
    try {
      ...
      const response = await retrieveClientVisitExport(
        token,
        clientId,
        startDateString,
        endDateString,
  ...
  const exportVisit = () => {
    ...
    link.setAttribute("download", `${clientName} Visit Logs.csv`);
    ...
    toast({
      ...
      description: (
        <>
          Visits for <strong>{clientName}</strong> have been successfully
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50 sticky top-0">
                    <tr className="border-b">
                      ...
                      <th className="px-4 py-3 text-left font-semibold text-gray-900">Staff Member</th>
                    ...
                    {previewData?.map((visit, index) => {
                      ...
                      return (
                        <tr key={index} className="border-b hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-gray-700">{staffName}</td>
...
export default ExportClientVisits;
