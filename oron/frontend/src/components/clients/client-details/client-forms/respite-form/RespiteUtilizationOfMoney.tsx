...
import {
  submitRespiteForm,
  submitRespiteUtilizationOfMoney,
} from "@/actions/clients/respite/respiteForm";
...
const RespiteUtilizationOfMoney = ({
  ...
  useEffect(() => {
    if (!respiteForm?.utilizationOfMoney) return;

    setMethod("PATCH");
    const { utilizationOfMoney } = respiteForm;
  ...
  const onSubmit = async (data: MoneyUtilizationFormData) => {
    try {
      ...
      const response = await submitRespiteUtilizationOfMoney(
        ...
        respiteForm.id,
        method,
        respiteForm?.utilizationOfMoney?.id
      );
      ...
      const submitResponse = await submitRespiteForm(token, respiteForm.id);
...
export default RespiteUtilizationOfMoney;
