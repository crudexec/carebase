...
const RespiteConcernsAndChallenges = ({
  ...
  useEffect(() => {
    if (!respiteForm?.concernAndChallenges) return;

    setMethod("PATCH");
    const { concernAndChallenges } = respiteForm;
  ...
  const onSubmit = async (data: ConcernsAndChallengesFormData) => {
    try {
      ...
      const response = await submitRespiteConcernsAndChallenges(
        ...
        respiteForm?.id || "",
        method,
        respiteForm?.concernAndChallenges?.id
      );
...
export default RespiteConcernsAndChallenges;
