...
const RespiteSensoryNeeds = ({
  ...
  useEffect(() => {
    if (!respiteForm?.sensoryNeedAndMotorDevelopment) return;

    setMethod("PATCH");
    const { sensoryNeedAndMotorDevelopment } = respiteForm;
  ...
  const onSubmit = async (data: SensoryNeedsFormData) => {
    try {
      ...
      const response = await submitRespiteSensoryNeeds(
        ...
        respiteForm.id,
        method,
        respiteForm?.sensoryNeedAndMotorDevelopment?.id
      );
...
export default RespiteSensoryNeeds;
