...
const RespiteBehaviourManagement = ({
  ...
  const getMappedBehaviours = useMemo(() => {
    if (!respiteForm?.behaviorManagement) return [];

    return respiteForm.behaviorManagement.map((behavior) => ({
  ...
  const handleAddBehaviour = async () => {
    if (respiteForm?.behaviorManagement?.length === 0) {
      append({
        behavior_type: "",
      ...
      try {
        ...
        const response = await submitRespiteBehaviourManagement(
          ...
          respiteForm?.id || "",
          "POST",
          respiteForm?.behaviorManagement?.[0]?.id
        );

        await refetchRespiteData();
  ...
  const onSubmit = async (data: BehaviourFormData) => {
    try {
      if (
        ...
        respiteForm?.behaviorManagement?.length === 0
      ) {
...
export default RespiteBehaviourManagement;
