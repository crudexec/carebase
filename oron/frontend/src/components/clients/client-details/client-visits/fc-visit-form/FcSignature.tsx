...
import {
  submitFcVisit,
  submitFcVisitSignatureForm,
} from "@/actions/clients/fc-visit/signature";
...
export type FcVisitSignatureFormSchema = z.infer<typeof formSchema>;
...
const FcSignature = ({
  ...
  } = useForm<FcVisitSignatureFormSchema>({
  ...
  useEffect(() => {
    ...
    const { treatmentPlanSignature } = data;
    ...
    if (
      ...
      setValue("name", treatmentPlanSignature?.full_name ?? "");
      setValue("signature_url", treatmentPlanSignature?.signature_url ?? "");
    }
  }, [visitForms, setValue]);
  ...
  const onSubmit = async (data: FcVisitSignatureFormSchema) => {
    try {
      const token = localStorage.getItem("token") as string;
      const response = await submitFcVisitSignatureForm(
        token,
        data,
        formId,
        visitForms?.data?.treatmentPlanSignature?.id ?? "-",
        method
      );
      ...
      const submitVisit = await submitFcVisit(token, formId);

      if (!submitVisit.status) {
        toast({
          variant: "destructive",
          description: submitVisit.errorMessage,
        });
        return;
      }
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[14px] font-[400] text-[#334155]">
            ...
            <span className="text-[#64748B] font-[700]">
              {calculateDuration(
                visitForms?.data?.start_time ?? "-",
                visitForms?.data?.end_time ?? "-"
              )}
            </span>{" "}
            <span>on</span>{" "}
            <span className="text-[#64748B] font-[700]">
