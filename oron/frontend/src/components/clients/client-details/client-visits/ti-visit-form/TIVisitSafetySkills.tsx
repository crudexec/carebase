...
const TIVisitSafetySkills = ({
  ...
  useEffect(() => {
    if (!tiForm?.safetyAndSurvivalSkills) return;

    setMethod("PATCH");
    const { safetyAndSurvivalSkills } = tiForm;
  ...
  const onSubmit = async (data: SafetySkillsFormData) => {
    try {
      ...
      const response = await submitRespiteSafetySurvivalSkills(
        ...
        tiForm?.id || "",
        method,
        tiForm?.safetyAndSurvivalSkills?.id
      );
              ...
              <Checkbox
                ...
                onCheckedChange={(checked) => {
                  ...
                  if (checked) {
                    ...
                    setValue("skills", [...currentSkills, "Other"]);
                  } else {
                    const currentSkills = getValues("skills");
                    setValue(
                      "skills",
...
export default TIVisitSafetySkills;
