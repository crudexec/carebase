...
const TIVisitPlayLeisure = ({
  ...
  useEffect(() => {
    if (!tiForm?.playLeisure) return;

    setMethod("PATCH");
    const { playLeisure } = tiForm;
    ...
    setValue("other_description", playLeisure.other_specify || "");
  }, [tiForm, setValue]);
  ...
  const onSubmit = async (data: LeisureFormData) => {
    try {
      ...
      const transformedData = {
        ...
        other_specify: data.other_description,
      };
  ...
  const showOtherInput = watchActivities.includes("other");
          ...
          {showOtherInput && (
            <Controller
              ...
              render={({ field }) => (
                <FormInput
                  {...field}
                  labelText="Please specify other activities"
...
export default TIVisitPlayLeisure;
