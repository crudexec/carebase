...
const sessionHighlightsSchema = z.object({
  ...
  tiOnsiteLocations: z
    .string()
    .min(1, { message: "Please select TI onsite locations" }),
  supervisor: z.string().min(1, { message: "Please select a supervisor" }),
});

export type TIVisitSessionHighlightsFormData = z.infer<
  typeof sessionHighlightsSchema
>;

const TIVisitSessionHighlights = ({
  ...
  } = useForm<TIVisitSessionHighlightsFormData>({
  ...
  useEffect(() => {
    if (!tiForm?.sessionHighlights) return;

    setMethod("PATCH");
    const { sessionHighlights } = tiForm;
    ...
    // setValue("tiOnsiteLocations", sessionHighlights.ti_onsite_locations || "");
    // setValue("supervisor", sessionHighlights.supervisor || "");
  }, [tiForm, setValue]);

  const onSubmit = async (data: TIVisitSessionHighlightsFormData) => {
    try {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-7">
        <div className="w-full flex flex-col xl:flex-row justify-between items-start gap-5">
          <Controller
            ...
            render={({ field }) => (
              <FormSelect
                labelText="This session occurred in"
        ...
        <div className="w-full flex flex-col xl:flex-row justify-between items-start gap-5">
          <Controller
            name="tiOnsiteLocations"
            control={control}
            render={({ field }) => (
              <FormSelect
                labelText="TI Onsite Locations"
                placeholder="Please select"
                selectContent={[
                ...
                value={field.value}
                disabled={isFormDisabled}
                isError={!!errors.tiOnsiteLocations}
                errorMessage={errors.tiOnsiteLocations?.message}
              />
            )}
          />
          ...
          <Controller
            ...
            render={({ field }) => (
              <FormSelect
                labelText="TI Onsite Supervisor"
                placeholder="Please select"
                selectContent={[
...
export default TIVisitSessionHighlights;
