...
const TIVisitPersonalCare = ({
  ...
  useEffect(() => {
    if (!tiForm?.personalCareAndBladderControl) return;

    setMethod("PATCH");
    const { personalCareAndBladderControl } = tiForm;
  ...
  const onSubmit = async (data: PersonalCareFormData) => {
    try {
      ...
      const transformedData = {
        ...
        other_personal_hygiene: selectOtherHygiene,
      ...
      const response = await submitRespitePersonalCare(
        ...
        tiForm?.id || "",
        method,
        tiForm?.personalCareAndBladderControl?.id
      );
...
export default TIVisitPersonalCare;
