  ...
  return (
    <div className="flex items-center gap-5 ml-auto">
      ...
      <Link
        ...
        href={`${row.original.route}/forms/intake?mode=edit`}
      >
        <EditIcon className="w-5 h-5 text-[#475569] font-[200]" />
      </Link>

      <button
        disabled={isPending}
        className="disabled:cursor-not-allowed text-[#475569] font-[200] disabled:text-gray-300"
        onClick={(e) => {
          e.stopPropagation();
          e.preventDefault();
      ...
      >
        {isPending ? <Loader height="h-fit" /> : <Trash className="w-5 h-5" />}
      </button>
    </div>
  );
};

export const draftColumns: ColumnDef<ClientsTableContainer>[] = [
  {
