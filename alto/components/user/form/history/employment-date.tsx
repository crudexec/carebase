import React from "react";

import { DateInput, FormField, FormRender, Input } from "@/components/ui";
import { cn } from "@/lib";
import { UserHistorySchema } from "@/schema/user/history";
import { ActionType, FormReturn } from "@/types";

type formType = FormReturn<typeof UserHistorySchema>;
const EmploymentDate = ({
  methods,
  mode,
}: {
  methods: formType;
  mode: ActionType;
}) => {
  return (
    <div
      className={cn(
        "grid grid-cols-1 gap-x-7 gap-y-4 md:px-8 px-4 items-end",
        "lg:grid-cols-2",
      )}
    >
      <FormField
        control={methods.control}
        name={`hireDate`}
        render={({ field }) => (
          <FormRender label={`Date of Hire`}>
            <DateInput
              onChange={field.onChange}
              value={field.value as Date}
              disabled={mode === "view"}
            />
          </FormRender>
        )}
      />
      <FormField
        control={methods.control}
        name={`lastDate`}
        render={({ field }) => (
          <FormRender label={`Last Date`}>
            <DateInput
              onChange={field.onChange}
              value={field.value as Date}
              disabled={mode === "view"}
            />
          </FormRender>
        )}
      />
      <FormField
        control={methods.control}
        name={`evaluationDueDate`}
        render={({ field }) => (
          <FormRender label={`3 Month Evaluation Due Date`}>
            <DateInput
              onChange={field.onChange}
              value={field.value as Date}
              disabled={mode === "view"}
            />
          </FormRender>
        )}
      />
      <FormField
        control={methods.control}
        name={"oigMonthlyUpdate"}
        render={({ field }) => (
          <FormRender label={"OIG Monthly Update"}>
            <Input
              {...field}
              value={field.value as string}
              disabled={mode === "view"}
            />
          </FormRender>
        )}
      />
      <FormField
        control={methods.control}
        name={`yearlyEvaluationDueDate`}
        render={({ field }) => (
          <FormRender label={`Yearly Evaluation Due Date`}>
            <DateInput
              onChange={field.onChange}
              value={field.value as Date}
              disabled={mode === "view"}
            />
          </FormRender>
        )}
      />
    </div>
  );
};

export default EmploymentDate;
