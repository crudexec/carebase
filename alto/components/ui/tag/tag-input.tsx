...
const TagInput = React.forwardRef<HTMLInputElement, TagInputProps>(
  (props, _ref) => {
    ...
    if (
      ...
      toast.error(
        "maxTags and minTags cannot be less than 0|Please set maxTags and minTags to a value greater than or equal to 0",
      );
      return null;
    }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      if (
        ...
        if (minLength && newTagText.length < minLength) {
          ...
          toast("Tag is too short|Please enter a tag with more characters");
          return;
        }

        // Validate maxLength
        if (maxLength && newTagText.length > maxLength) {
          toast("Tag is too long|Please enter a tag with less characters");
          return;
        }
    ...
    return (
      <div
        className={`w-full flex ${inputFieldPostion === "bottom" ? "flex-col" : inputFieldPostion === "top" ? "flex-col-reverse" : "flex-row"}`}
      >
        {!usePopoverForTags ? (
          <TagList
            tags={truncatedTags}
          ...
          <div className="w-full">
            {!usePopoverForTags ? (
              <Input
                ...
                disabled={
                  (maxTags !== undefined && tags.length >= maxTags) || disabled
                }
              />
            ) : (
              <TagPopover
